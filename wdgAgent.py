...
class WdgAgent:
    ...
    def __init__(self, metabolismMean, visionMean, title="Agents' metabolism and vision means", width=600, height=300):
        ...
        while True:
            try:
                self.metabolismSeries.append(next(seriesItr))
        ...
        while True:
            try:
                self.visionSeries.append(next(seriesItr))
            except StopIteration:
                break
        ...
        self.canvas.create_text(self.metabolismSeries[-1][-2], y0 - 10 - metabolismMean[-1] * incry,
                                text=metabolismMean[-1], fill='blue', anchor=SW)
